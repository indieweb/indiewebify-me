...
<div class="row demo-row">
    <div class="span12">
        ...
        <form class="row" action="/validate-h-card/" method="get">
            <div class="span4">
                <input type="text" id="validate-h-card-url" name="url" value="<?php echo $url ?? '' ?>" placeholder="https://example.com/" class="span4" required />
            </div>
            <div class="span3">
    ...
    <?php if ($showResult) : ?>
        ...
        <?php elseif (count($allHCards) > 0) :
            ...
            <h3> Almost there! </h3>
            <p> <?php echo $intro_phrase?> marking it up as the <a href="https://microformats.org/wiki/representative-h-card-authoring">representative h-card</a>. </p>
            <p> To identify the h-card that <em>represents</em> the page, you can: </p>
            <ul>
                <li> Add <code>class=&quot;u-url u-uid&quot;</code> on the h-card’s link to <?php echo htmlspecialchars($url);?> </li>
                <li> <b>Or:</b> add <code>class=&quot;u-url&quot; rel=&quot;me&quot;</code> on the h-card’s link to <?php echo htmlspecialchars($url);?> </li>
            </ul>
        ...
        <?php else: ?>
            ...
            <pre><code>&lt;a href=&quot;<?php echo $url ?>&quot; class=&quot;h-card&quot; rel=&quot;me&quot;>Your Name&lt;/a&gt;</code></pre>
        ...
        <?php if ($hCard) : ?>
            ...
            <?php if (Mf2\hasProp($hCard, 'photo')) : ?>
                <img class="photo-block" src="<?php echo Mf2\getProp($hCard, 'photo')?>" alt="" />
                <?php elseif (Mf2\hasProp($hCard, 'logo')) : ?>
                <img class="logo-block" src="<?php echo Mf2\getProp($hCard, 'logo')?>" alt="" />
                <?php else: ?>
                ...
                <?php endif ?>
                <p class="p-name"><?php echo Mf2\getProp($hCard, 'name') ?></p>
            ...
            <?php if (Mf2\hasProp($hCard, 'url')) : $urls = Mf2\getPlaintextArray($hCard, 'url'); ?>
                ...
                <?php foreach ($urls as $pUrl): ?>
                    <li><a href="<?php echo $pUrl ?>"><?php echo $pUrl ?></a></li>
                <?php endforeach ?>
                </ul>
                <?php else: ?>
            ...
            <?php if (Mf2\hasProp($hCard, 'note')) : ?>
                ...
                <p><?php echo Mf2\getProp($hCard, 'note') ?></p>
                <?php else: ?>
                <div class="empty-property-block">
