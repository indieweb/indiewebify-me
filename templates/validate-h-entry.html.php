...
<div class="row demo-row">
    ...
    <form class="row" action="/validate-h-entry/" method="get">
        <div class="span4">
            <input type="text" id="validate-h-entry-url" name="url" value="<?php echo $url ?>" placeholder="http://yoursite.com/notes/123456" class="span4" />
        </div>
        <div class="span3">
    ...
    <div class="result alert <?php if ($error) : ?>alert-warning<?php else: ?>alert-success<?php 
        ...
        <?php if ($error) : ?>
            ...
            <p>When fetching <code><?php echo $url ?></code>, we encountered this problem:</p>
            <p><?php echo $error['message'] ?></p>
        <?php elseif ($hEntry !== null) : ?>
            <h4>Success!</h4>

            <p>We found the following <strong><?php echo $postType ?></strong> <code>h-entry</code> on your site:</p>

            <div class="preview-h-entry preview-block">

            <?php if ($nameState == 'valid') : ?>
                <p class="property-block-name">Name</p>
                <p class="p-name"><?php echo Mf2\getProp($hEntry, 'name') ?></p>
            ...
            <?php if (Mf2\hasProp($hEntry, 'author')) : $author = $hEntry['properties']['author'][0]; ?>
                ...
                <div class="minicard p-author">
                    ...
                    <img class="u-photo" src="<?php echo Mf2\getProp($author, 'photo')?>" alt="" />
                    <?php else: ?>
                    ...
                    <?php endif ?>
                    <span class="p-name"><?php echo Mf2\getProp($author, 'name') ?></span>
                    <a href="<?php echo Mf2\getProp($author, 'url') ?>" rel="author" class="u-url"><?php echo Mf2\getProp($author, 'url') ?></a>
                </div>
            ...
            <?php if ($postType == 'reply') : ?>
                ...
                <?php foreach ($hEntry['properties']['in-reply-to'] as $irt): ?>
                    ...
                    <?php if (is_string($irt)) : ?>
                                <a href="<?php echo $irt ?>"><?php echo $irt ?></a>
                            <?php elseif (Mf2\isMicroformat($irt)) : ?>
                                ...
                                <?php if (Mf2\hasProp($irt, 'url')) : ?>
                                    <a href="<?php echo Mf2\getProp($irt, 'url') ?>"><?php echo Mf2\getProp($irt, 'url') ?></a>
                                <?php else: ?>
                ...
                <?php elseif ($postType == 'like') : ?>
                    ...
                    <?php if (is_string($hEntry['properties']['like-of'][0])) : ?>
                        <a href="<?php echo Mf2\getProp($hEntry, 'like-of') ?>"><?php echo Mf2\getProp($hEntry, 'like-of') ?></a>
                    <?php elseif (Mf2\isMicroformat(Mf2\getProp($hEntry, 'like-of'))) : ?>
                        ...
                        <?php if (Mf2\hasProp(Mf2\getProp($hEntry, 'like-of'), 'url')) : ?>
                            <a href="<?php echo Mf2\getProp(Mf2\getProp($hEntry, 'like-of'), 'url') ?>"><?php echo Mf2\getProp(Mf2\getProp($hEntry, 'like-of'), 'url') ?></a>
                        <?php else: ?>
                            <p>Give the nested microformat a URL property! <code class="pull-right">&lt;a class="u-url" href="…">&lt;/a></code></p>
                        <?php endif ?>
                    <?php else: ?>
                ...
                <?php elseif ($postType == 'repost') : ?>
                    ...
                    <?php if (is_string(Mf2\getProp($hEntry, 'repost-of'))) : ?>
                        <a href="<?php echo Mf2\getProp($hEntry, 'repost-of') ?>"><?php echo Mf2\getProp($hEntry, 'repost-of') ?></a>
                    <?php elseif (Mf2\isMicroformat(Mf2\getProp($hEntry, 'repost-of'))) : ?>
                        ...
                        <?php if (Mf2\hasProp(Mf2\getProp($hEntry, 'repost-of'), 'url')) : ?>
                            <a href="<?php echo Mf2\getProp(Mf2\getProp($hEntry, 'repost-of'), 'url') ?>"><?php echo Mf2\getProp(Mf2\getProp($hEntry, 'repost-of'), 'url') ?></a>
                        <?php else: ?>
                            <p>Give the nested microformat a URL property! <code class="pull-right">&lt;a class="u-url" href="…">&lt;/a></code></p>
                        <?php endif ?>
                    <?php else: ?>
            ...
            <?php if (Mf2\hasProp($hEntry, 'content')) : ?>
                <div class="e-content"><?php echo Mf2\getProp($hEntry, 'content') ?></div>
            ...
            <?php if (Mf2\hasProp($hEntry, 'published')) : ?>
                <time class="dt-published"><?php echo Mf2\getProp($hEntry, 'published') ?></time></p>
                <?php if (datetimeProblem(Mf2\getProp($hEntry, 'published'))) : ?>
                    <?php echo datetimeProblem(Mf2\getProp($hEntry, 'published')) ?>
                <?php endif ?>
                <?php else: ?>
                </p>
                <div class="empty-property-block">
            ...
            <?php if (Mf2\hasProp($hEntry, 'url')) : ?>
                <a href="<?php echo Mf2\getProp($hEntry, 'url') ?>"><?php echo Mf2\getProp($hEntry, 'url') ?></a></p>
                <?php else: ?>
  ...
  &lt;a class=&quot;u-url&quot; href=&quot;https://example.com/my-first-post&quot;>
    Published &lt;time class=&quot;dt-published&quot;><?php echo date('Y-m-d H:i:sO') ?>&lt;/time>
  &lt;/a>
&lt;/article></code></pre>
        <?php endif ?>
